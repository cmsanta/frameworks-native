...
void ApplyFramebufferAttachmentCMAAINTELResourceManager::Initialize(){
	GLubyte *str = GetGLVersionInfo();
//	fprintf(stderr,"GLVersion from CMMA: %s\n",str);
	if (strstr ((const char *) str, "3.2")) {
		//fprintf(stderr,"GLES v: %s",str);
		is_gles31_compatible_ = true;
	}
	...
	if(is_gles31_compatible_) {
	   ...
	   //     "GL_ARB_shader_image_load_store"));
	    ...
	    {
	      fprintf(stderr,"********actually being used\n");
	      GL_CHECK(glActiveTexture(GL_TEXTURE0));

	      GLuint rgba8ui_texture = 0, depth_texture = 0;
	      GL_CHECK(glGenTextures(1, &rgba8ui_texture));
	      GL_CHECK(glBindTexture(GL_TEXTURE_2D, rgba8ui_texture));
	      GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8UI, 4, 4));

	      GL_CHECK(glGenTextures(1, &depth_texture));
	      GL_CHECK(glBindTexture(GL_TEXTURE_2D, depth_texture));
	      GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT16, 4, 4));

	      // Create the FBO
	      GLuint rgba8ui_framebuffer = 0;
	      GL_CHECK(glGenFramebuffers(1, &rgba8ui_framebuffer));
	      GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, rgba8ui_framebuffer));
	      ...
	      GL_CHECK(glDeleteFramebuffers(1, &rgba8ui_framebuffer));
	      GL_CHECK(glDeleteTextures(1, &rgba8ui_texture));
	      GL_CHECK(glDeleteTextures(1, &depth_texture));
	    }
	  }

	fprintf(stderr, "ApplyFramebufferAttachmentCMAAINTEL: \n");
	fprintf(stderr, "Supports USampler is: %d \n",supports_usampler_);
    fprintf(stderr, "ApplyFramebufferAttachmentCMAAINTEL: \n");
	fprintf(stderr, "Supports R8 Images is %d\n ",supports_r8_image_);
...
GLubyte* ApplyFramebufferAttachmentCMAAINTELResourceManager::GetGLVersionInfo(){
	...
	while(*strVersion){
		if((*strVersion <= '9') && (*strVersion >='0') || (*strVersion =='.') ) {
			//fprintf(stderr,"GL_VERSION string..: %c\n",*strVersion);
			strShortVersion[i++] = *strVersion;
			if(j==2)
				break;
			j++;
		}
...
void ApplyFramebufferAttachmentCMAAINTELResourceManager::
  ...
  if (! initialized_){
	fprintf(stderr, "problem during initialization?\n");
	return;
  }
  ...
  for (uint32_t i = 0; i < max_draw_buffers; i++) {
	  ...
	  //FIXME: bring rgba_immutable
      const bool do_copy = 1;//!rgba_immutable;
...
void ApplyFramebufferAttachmentCMAAINTELResourceManager::ApplyCMAAEffectTexture(
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) {
    fprintf(stderr,  "ApplyFramebufferAttachmentCMAAINTEL: \n");
    fprintf(stderr, "Incomplete framebuffer: status: 0x%x line: %d\n",status,__LINE__);
    Destroy();
    return;
  }
#endif
  ...
  GL_CHECK(glViewport(0, 0, (width_ + 1) / 2, (height_ + 1) / 2));
  ...
  {
    GL_CHECK(glUseProgram(edges0_shader_));
	
    GL_CHECK(glUniform2f(0, 1.0f / width_, 1.0f / height_));
    GL_CHECK(glDepthMask(GL_TRUE));
    GL_CHECK(glDepthFunc(GL_ALWAYS));
    GL_CHECK(glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE));

    if (!is_gles31_compatible_) {
      GL_CHECK(glUniform1i(edges0_shader_result_rgba_texture_slot1_, 1));
      GL_CHECK(glUniform1i(edges0_shader_target_texture_slot2_, 2));
    }
    GL_CHECK(glBindImageTexture(1, working_color_texture_, 0, GL_FALSE, 0,
                          GL_WRITE_ONLY, GL_RGBA8));
	
    if (do_copy) {
      GL_CHECK(glUniform1i(2, GL_TRUE));
      GL_CHECK(glBindImageTexture(2, dest_texture, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                            GL_RGBA8));
    } else {
      GL_CHECK(glUniform1i(2, GL_FALSE));
    }

    GL_CHECK(glActiveTexture(GL_TEXTURE0));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, source_texture));
  ...
  {
    GL_CHECK(glUseProgram(edges1_shader_));
    ...
    if (!is_gles31_compatible_) {
      GL_CHECK(glUniform1i(edges1_shader_result_edge_texture_, 0));
    }
    GL_CHECK(glBindImageTexture(0, edge_texture_b, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                          edge_format));


    GL_CHECK(glActiveTexture(GL_TEXTURE1));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, mini4_edge_texture_));
  ...
  //  gl_FragDepth                        mini4_edge_texture_          fbo.depth
  {
    ...
    GL_CHECK(glUseProgram(edges_combine_shader_));
    GL_CHECK(glUniform2f(0, 1.0f / width_, 1.0f / height_));
    GL_CHECK(glDepthMask(GL_TRUE));
    GL_CHECK(glDepthFunc(GL_LESS));
    GL_CHECK(glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE));
    ...
    if (!is_gles31_compatible_) {
      GL_CHECK(glUniform1i(edges_combine_shader_result_edge_texture_, 0));
    }
    GL_CHECK(glBindImageTexture(0, edge_texture_a, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                          edge_format));

    GL_CHECK(glActiveTexture(GL_TEXTURE1));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, edge_texture_b));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
  ...
  //  gl_FragDepth                         mini4_edge_texture_         fbo.depth
  {
    GL_CHECK(glUseProgram(process_and_apply_shader_));
    GL_CHECK(glUniform2f(0, 1.0f / width_, 1.0f / height_));
    GL_CHECK(glDepthMask(GL_FALSE));
    GL_CHECK(glDepthFunc(GL_LESS));
    GL_CHECK(glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE));
    ...
    if (!is_gles31_compatible_) {
      GL_CHECK(glUniform1i(process_and_apply_shader_result_rgba_texture_slot1_, 1));
    }
    GL_CHECK(glBindImageTexture(1, dest_texture, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                          GL_RGBA8));

    GL_CHECK(glActiveTexture(GL_TEXTURE0));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, working_color_texture_));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));

    GL_CHECK(glActiveTexture(GL_TEXTURE1));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, edge_texture_a));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
...
void ApplyFramebufferAttachmentCMAAINTELResourceManager::OnSize(GLint width,
                                                                GLint height, GLuint *rgba8_texture_main) {
  ...
  *rgba8_texture_main = rgba8_texture_;
  ...
  GLenum edge_format = supports_r8_image_ ? GL_R8 : GL_R32F;
  GL_CHECK(glGenTextures(1, &edges0_texture_));
  GL_CHECK(glBindTexture(GL_TEXTURE_2D, edges0_texture_));
  GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, edge_format, width, height));

  GL_CHECK(glGenTextures(1, &edges1_texture_));
  GL_CHECK(glBindTexture(GL_TEXTURE_2D, edges1_texture_));
  GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, edge_format, width, height));

  // Color working texture - RGBA8
  GL_CHECK(glGenTextures(1, &working_color_texture_));
  GL_CHECK(glBindTexture(GL_TEXTURE_2D, working_color_texture_));
  GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, width, height));
  ...
  GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, format, (width + 1) / 2,
                    (height + 1) / 2));
  ...
  GL_CHECK(glBindTexture(GL_TEXTURE_2D, mini4_edge_depth_texture_));
  GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
		  width, height,0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
...
GLuint ApplyFramebufferAttachmentCMAAINTELResourceManager::CreateShader(
  ...
  if (compile_result == 0) {
	...
	if(type == GL_VERTEX_SHADER)
		...
		fprintf(stderr,"GL_FRAGMENT_SHADER\n");
	else
