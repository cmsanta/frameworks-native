...
GLuint programID = 0;
GLuint programIDFilter = 0;
GLint iLocPosition = -1;
GLint iLocTextureMix = -1;
GLint iLocTexture = -1;
GLint iLocTextureFilter = -1;
...
GLuint iFBOTex_src = 0;
GLuint iFBOTex_copy_from_fbo = 0;
GLuint rgba8_texture_main = 0;
GLuint rgba8_texture_main_final = 0;

//FIXME: global for now...
int filter_cmma = 0, gl_copy = 0, gl_texelFetch = 0;
        ...
        if (!compiled) {
            ...
            if (infoLen) {
                ...
                if (buf) {
                    ...
                    fprintf(stderr, "Could not compile shader 0x%x:\n%s\n",
                            shaderType, buf);
                    free(buf);
                }
...
static const char copy_frag_str_[] =
    ...
    "    vec4 test = vec4(0.2,0.0,0.29,0.1) + pixel;\n"
    "    outColor = pixel;\n"
    "}\n\n";
...
bool setupGraphics(int w, int h) {
	...
	/*FIXME: have this created only inside if/else:  final Filter at the end */
    programIDFilter = createProgram(vert_str_, copy_frag_str_);
    if (!programIDFilter) {
		fprintf(stderr,"error creating shader programs\n");
        return false;
    }
    ...
    GL_CHECK(glGenTextures(1, &iFBOTex_src));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, iFBOTex_src));
    /* Set filtering. */
	...
	/**/
    GL_CHECK(glGenTextures(1, &rgba8_texture_main_final));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, rgba8_texture_main_final));
	GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, FBO_WIDTH, FBO_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
    /* Set filtering. */
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));	
    ...
    /* Initialize FBOs. */
    GL_CHECK(glGenFramebuffers(1, &iFBO_final));
    ...
    GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, iFBOTex_src, 0));
    ...
    else
    {
  	...
  	/* need program to check status of shader attributes */
	GL_CHECK(glUseProgram(programIDFilter));

    /* Texture. */
    iLocTextureFilter = GL_CHECK(glGetUniformLocation(programIDFilter, "inTexture"));
    if(iLocTextureFilter == -1)
    {
        fprintf(stderr,"Warning: Uniform not found at %s:%i\n", __FILE__, __LINE__);
    }
    else
    {
        GL_CHECK(glUniform1i(iLocTextureFilter, 0));
    }
...
void renderFrame(GLint w, GLint h) {
	...
	if(filter_cmma) {
		cmma->ApplyFramebufferAttachmentCMAAINTEL(&iFBOTex_src, &iFBOTex_copy_from_fbo, TEXTURE_WIDTH, TEXTURE_HEIGHT, GL_RGBA);
	}
	...
	if(gl_copy){
		...
		GL_CHECK(glBindTexture(GL_TEXTURE_2D, iFBOTex_copy_from_fbo));
  		...
  		GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
  		GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
  		GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, iFBO_final));
  		GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                            iFBOTex_copy_from_fbo, 0));
#if 1
  GLenum fb_status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
  if (GL_FRAMEBUFFER_COMPLETE != fb_status) {
    fprintf(stderr,"CopyTextureCHROMIUM: Incomplete framebuffer.");
  }
#endif
		/**/
		GL_CHECK(glBindTexture(GL_TEXTURE_2D, rgba8_texture_main_final));
		...
		//GL_CHECK(glBindTexture(GL_TEXTURE_2D, /*iFBOTex_copy_from_fbo*/rgba8_texture_main));
		GL_CHECK(glGenerateMipmap(GL_TEXTURE_2D));
		GL_CHECK(glBindTexture(GL_TEXTURE_2D, 0));

	    }else if(gl_texelFetch){

		
  		GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, iFBO_final));
  		GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                            rgba8_texture_main_final, 0));
#if 1
		GLenum fb_status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
		if (GL_FRAMEBUFFER_COMPLETE != fb_status) {
		  fprintf(stderr,"CopyTextureCHROMIUM: Incomplete framebuffer.");
...
int main(int argc, char** argv) {
	...
	while ((c = getopt (argc, argv, "cbth")) != -1)
	switch(c) {
		case 'c':
			//CMMA filter
			filter_cmma = 1;
		...
		case 'b':
			...
			break;
		case 't':
			//copy through texelFetch/glDrawArray
			gl_texelFetch = 1;
			break;
		case 'h':
			//display help menu
	...
	if( (gl_copy&&gl_texelFetch) || (gl_texelFetch&&!gl_copy&&!filter_cmma)){
		fprintf(stderr,"Invalid input parameters: glCopy: %d, gl_texelFetch: %d\n",gl_copy, gl_texelFetch);
		fprintf(stderr, "Run ./cmma-test -h for all valid options.\n");
	...
	if(help_menu){
		fprintf(stderr, "\n**** Help menu **** \n\n");
		fprintf(stderr, "This unit test implements the CMAA filtering technique and demonstrates\n");
		fprintf(stderr, "2 different approaches when copying data out of the fbo into another one\n");
		fprintf(stderr, "for display into the screen.\n");
		fprintf(stderr, "\nMethod #1: glCopyTexSubImage2D() by ./cmma-test -c -b\n");
		fprintf(stderr, "Method #2: texelFetch() inside a fragment shader by ./cmma-test -c -t \n\n");
		fprintf(stderr, "Exiting...\n");
		return 0;
	}

	fprintf(stderr,"filter: %d, gl_copy: %d, gl_texelFetch: %d\n",filter_cmma, gl_copy, gl_texelFetch);
   ...
   glDeleteProgram(programIDFilter);
